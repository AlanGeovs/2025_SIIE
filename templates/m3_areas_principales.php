<form id="formAreasPrincipales" class="p-3">
...
<script>
    // === Configuración de áreas principales ===
    const areasPrincipales = ["Baño", "Dirección", "Letrina", "Salón"];
    ...
    function obraEnProcesoBlock(slug) {
        ...
        const nameObra = `ap_obra_${slug}`;
        const idBase = `ap_obra_${slug}`;
        const tipoObraId = `tipo-obra-${slug}`;
        const recursoObraId = `recurso-obra-${slug}`;
        const nameTipoObra = `ap_tipo_obra_${slug}`;
        const nameRecursoObra = `ap_recurso_obra_${slug}`;
    ...
    function evidenciaButton(slug) {
        ...
        <label class="btn btn-evidencia btn-sm w-100">
            Agregar evidencia
            <input type="file" class="evidencia-input" data-modulo="3-areas-principales-${slug}" style="display:none;">
        </label>`;
    }

    // Construye una fila para un área principal
    function crearFilaAreaPrincipal(area) {
        const slug = slugify(area);
        return `
        <tr>
            <td class="text-start">${area}</td>
            <td>${radiosSiNo('ap_existe_' + slug)}</td>
            <td>${selectTipoConstruccion('ap_tipo_construccion_' + slug)}</td>
            <td>${selectCantidad('ap_cantidad_' + slug)}</td>
            <td>${radiosSiNo('ap_uso_' + slug)}</td>
            <td>${selectCondicion('ap_condicion_' + slug)}</td>
            <td>${radiosSiNo('ap_danio_estructural_' + slug)}</td>
            <td>${radiosSiNo('ap_danio_instalacion_' + slug)}</td>
            <td>${obraEnProcesoBlock(slug)}</td>
            <td>${radiosSiNo('ap_construccion_adicional_' + slug)}</td>
            <td>${evidenciaButton(slug)}</td>
        </tr>`;
    }

    // Render de tabla (solo áreas principales)
    document.addEventListener("DOMContentLoaded", () => {
        const tbody = document.getElementById("tablaAreasPrincipales");
        tbody.innerHTML = "";
        areasPrincipales.forEach(area => {
            tbody.insertAdjacentHTML("beforeend", crearFilaAreaPrincipal(area));
        });
