...
<script>
    // Lista de equipo (según especificación)
    const equipo = [
        "Aire acondicionado",
        "Bocina para PC",
        "Computadora PC",
    ...
    function generarFila(nombre, tipo) {
        const id = `${tipo}-${nombre.replace(/\s+/g, '-').toLowerCase()}`;
        ...
        <tr>
            ...
            <td>
                <label class="btn btn-evidencia btn-sm w-100" style="background-color:#691c32; color:#fff;">
                    Agregar evidencia
                    <input type="file" class="evidencia-input" data-modulo="3-equipo-${nombre.replace(/\s+/g, '-').toLowerCase()}" style="display:none;">
                </label>
            </td>
        </tr>`;
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Exclusividad de radios dentro de cada grupo name
